...
func TestConsumer_Load(t *testing.T) {
	...
	baseCfg := rmq.CommonConfig{Log: logf}
	...
	rmqConn := rmq.ConnectWithAMQPConfig(ctx, rmq.ConnectConfig{CommonConfig: baseCfg, Topology: topology}, os.Getenv("TEST_AMQP_URI"), amqp.Config{})
	...
	publisher := rmq.NewPublisher(ctx, rmqConn, rmq.PublisherConfig{CommonConfig: baseCfg})
...
func TestRMQConsumer_AutogeneratedQueueNames(t *testing.T) {
	...
	cons := rmq.NewConsumer(rmqConn, rmq.ConsumerConfig{
		CommonConfig: baseCfg,
	...
	publisher := rmq.NewPublisher(ctx, rmqConn, rmq.PublisherConfig{CommonConfig: baseCfg, LogReturns: true})
	pubCount := 10
	time.Sleep(time.Second / 3)
